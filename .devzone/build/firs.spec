...
for i in $(ls -1 sprites/nfo/lang/ | grep '.pnfo$' | grep -v '.new.' | grep -v 'cleanup' | grep -v 'nfo_lang' | grep -v 'untranslated' | grep -v 'remove'); do ./scripts/check_language.sh $i >$i.log; [[ $(cat $i.log | wc -l) -lt 4 ]] && rm $i.log; done
