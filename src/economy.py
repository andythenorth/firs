        ...
        for counter, cargo in enumerate(cargos_by_price_factor):
            if counter > 0:
                ...
                if result[previous_cargo.id] >= cargo.price_factor:
                    ...
                    utils.echo_message('Cargo ' + cargo.id + ' has overlapping price_factor with ' + previous_cargo.id + ' in economy ' + self.id + '; automatically adjusting (this may or not need changing).', 'info')
